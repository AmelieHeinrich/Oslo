...
// > Create Time: 2025-04-06 16:07:57
//
...
RaytracingPipeline::RaytracingPipeline(RaytracingPipelineSpecs specs)
{
    if (!specs.Signature) {
    ...
    HRESULT result = RHI::GetDevice()->GetDevice()->CreateStateObject(&desc, IID_PPV_ARGS(&mPipeline));
    if (FAILED(result)) {    
        LOG_CRITICAL("Failed to create raytracing pipeline!");
    }

   mIDBuffer = std::make_shared<Buffer>(3 * D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT, D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT, BufferType::Constant, "ID Buffer");
