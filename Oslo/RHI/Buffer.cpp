...
Buffer::Buffer(uint64_t size, uint64_t stride, BufferType type, const std::string& name)
    : Resource(), mType(type)
{
    mShouldFree = true;
    mSize = size;
...
Buffer::~Buffer()
{
    RHI::GetHeaps()[DescriptorHeapType::ShaderResource]->Free(mCBV);
    RHI::GetHeaps()[DescriptorHeapType::ShaderResource]->Free(mUAV);
    RHI::GetHeaps()[DescriptorHeapType::ShaderResource]->Free(mSRV);
}
...
void Buffer::BuildCBV()
{
    ...
    if (mCBV.Valid == false)
        mCBV = RHI::GetHeaps()[DescriptorHeapType::ShaderResource]->Allocate();
    RHI::GetDevice()->GetDevice()->CreateConstantBufferView(&cbvd, mCBV.CPU);
}
...
void Buffer::BuildUAV()
{
    ...
    if (mUAV.Valid == false)
        mUAV = RHI::GetHeaps()[DescriptorHeapType::ShaderResource]->Allocate();
    RHI::GetDevice()->GetDevice()->CreateUnorderedAccessView(mResource, nullptr, &uavd, mUAV.CPU);
}
...
void Buffer::BuildSRV()
{
    ...
    RHI::GetDevice()->GetDevice()->CreateShaderResourceView(mResource, &srv, mSRV.CPU);
}
...
void Buffer::CopyMapped(void *data, uint64_t size)
{
    void* mapped = nullptr;
    Map(0, 0, &mapped);
    if (mapped) {
