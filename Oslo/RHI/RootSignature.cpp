...
// > Create Time: 2025-04-06 15:41:40
//
...
RootSignature::RootSignature()
{
    ...
    HRESULT Result = RHI::GetDevice()->GetDevice()->CreateRootSignature(0, pRootSignatureBlob->GetBufferPointer(), pRootSignatureBlob->GetBufferSize(), IID_PPV_ARGS(&mRootSignature));
    ASSERT(SUCCEEDED(Result), "Failed to create root signature!");
    pRootSignatureBlob->Release();
}

RootSignature::RootSignature(const std::vector<RootType>& roots, uint64_t pushConstantSize)
{
